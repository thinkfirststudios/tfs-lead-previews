/* ============================================================
   Haul It All Junk Removal — interactions
   The load-scale is the page: selecting a tier animates the truck fill
   and carries the choice down to the booking form, so the visitor never
   re-enters what they already told us.
   ============================================================ */
(function () {
  'use strict';
  ...
  (function initTiers() {
    ...
    var field = document.getElementById('tierField');
    if (!tiers.length) return;
    tiers.forEach(function (t) {
      t.addEventListener('click', function () {
        tiers.forEach(function (x) { x.setAttribute('aria-pressed', x === t ? 'true' : 'false'); });
        if (fill) fill.setAttribute('width', t.getAttribute('data-fill'));
        if (field) field.value = t.getAttribute('data-tier');
      });
    });
  })();

  /* ---------- same-day / next-day toggle ---------- */
  (function initDays() {
    var btns = Array.prototype.slice.call(document.querySelectorAll('.day-toggle button'));
